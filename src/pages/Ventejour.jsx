      ...
      try {
        ...
        const groupedByStore = todayRows.reduce((acc, row) => {
          ...
          products.forEach((product) => {
            // Check if this product already exists in the order's products list
            const existingProduct = acc[store][row.order_id].products.find(
              (p) => p.product_id === product.product_id
            );

            // Get the product name either from the order data or from the lookup map
            const productName =
        ...
        console.error("Failed to fetch orders and products:", err);
