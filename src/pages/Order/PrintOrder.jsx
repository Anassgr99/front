...
const PrintOrder = () => {
    ...
    const fetchData = async () => {
      try {
        ...
        const orderResponse = await axios.get(
          `http://192.168.1.7:3000/api/Orders/${orderId}`
        );
        const orderData = orderResponse.data;
        console.log(orderData);

        // Ensure that order.products is parsed as an array if it's a string
        ...
        if (orderData.customer_id) {
          const customerResponse = await axios.get(
            `http://192.168.1.7:3000/api/customers/${orderData.customer_id}`
          );
  ...
  const generatePDF = () => {
    ...
    doc.text(`Order Status: ${order?.order_status || "N/A"}`, 10, 140);
    ...
    order.products.forEach((product, index) => {
      ...
      // Draw the cells for each product
          ...
          {customer ? (
            <>
              <p className="flex items-center gap-2">
                <FiUser className="text-gray-600" /> <strong>Nom:</strong>{" "}
                {customer.name || "N/A"}
              </p>
              <p className="flex items-center gap-2">
                <FiPhone className="text-gray-600" /> <strong>Téléphone:</strong>{" "}
                {customer.phone || "N/A"}
              </p>
              <p className="flex items-center gap-2">
                <FiMapPin className="text-gray-600" /> <strong>Address:</strong>{" "}
